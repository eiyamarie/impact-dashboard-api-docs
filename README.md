...
- On **first creation** of a company, app invites ("set your password" links) are sent to the primary `email` and every address in `ownerEmails` / `owner_emails` automatically, never to a rep's email. Invite links expire after 72 hours. Repeat deliveries (same `contactid`) update the company and never re-invite, and if an owner email is already a login no second invite is sent. Invites are best-effort and do not change the response: creation still returns `201` even if an invite email fails. Disable globally with `B2B_AUTO_INVITE=off`.
...
Stores a client's 1-on-1 booking form submission (the Google Form for the 2nd, 3rd, and 4th 1-on-1 calls) so Elle or Mary can approve or deny it on the client's profile (anyone else who can see the client can view requests but not decide). Each stored request also sends an in-app notification linking straight to the request card, targeted at Elle and Mary plus any extra addresses in `ONE_ON_ONE_NOTIFY_EMAILS` (falling back to everyone holding the `approveRequests` permission when none of those emails match a user). Posting to Discord is the sender's job; this endpoint only records the request in the dashboard. On approval the dashboard mints a single-use Cal.com booking link and (when configured) pushes the decision to the outbound decision webhook described below; the booked call should then be recorded through the existing `POST /api/webhooks/contacts/{contactId}/calls` endpoint with `call_type: one_on_one`, which is what the profile's 1-on-1 counter is derived from.
...
Each stored request notifies the approvers listed in `REFUND_APPROVER_EMAILS`, in-app and by email, with a link straight to the request card. When that variable is unset or matches no dashboard user holding the `approveRequests` permission, the in-app notification falls back to everyone holding it, so a pending refund is never announced to nobody. After a decision, the requester is notified by email, and in-app when `requester_email` matches a dashboard user.
...
**Two accepted bodies. Whop's own webhook envelope is the one to send.** Point a Whop webhook subscription at this URL through anything that can add the `x-api-key` header, and forward the body unchanged: `[{ "id": "msg_...", "type": "refund.created", "timestamp": "...", "data": { "status": "succeeded", "payment": { "id": "pay_...", "user": { "id": "user_..." } }, "amount": 50, "currency": "usd" } }]`. Nothing is mapped by hand, which is the point; the first sender that mapped it by hand got four of four fields wrong. Whop posts an ARRAY of messages even for one event, and a bare object is accepted too. Unknown keys on Whop's nested objects are ignored (the one lenient exception on this route, see `docs/INVARIANTS.md` §6).

Subscribe to `refund.created`, `refund.updated`, `dispute.created` and `dispute.updated`. On Whop's envelope, `dispute_alert.created` and any dispute in a `warning_*` status is acknowledged and NOT recorded (on the flat body below a `warning_*` status is a `400` instead, because that shape has no way to say "acknowledged"): an early warning is not a chargeback. **Statuses are Whop's own words, stored unchanged.** Refunds: `pending`, `requires_action`, `succeeded`, `failed`, `canceled` (one l, as Whop spells it). Disputes: `needs_response`, `under_review`, `won`, `lost`, `closed`, `other`. There is no translation layer, so there is nothing to keep in step. A handled event whose status is not in that list is a `400` naming the status, never a silent drop. `data.payment` is nullable in Whop's schema but required here: an undo is matched to its refund by payment, so a payment-less event could never be paired. Whop sends `amount` as a decimal in the currency, which is what this endpoint expects; Whop's OpenAPI spec describes it as "the refunded amount as a decimal in the specified currency, such as 10.43 for $10.43 USD", and a real `refund.created` payload carried `50` for a $50.00 refund.

The flat body below is also still accepted, for a sender that prefers to map the fields itself. It is `.strict()`: unknown keys are a `400`.

The endpoint uses the standard `x-api-key` webhook authentication. `event_id` is the provider's immutable event ID and must be supplied again on retries. On Whop's envelope it is the envelope's `id` (`msg_...`), NOT the refund's `id` (`rf_...`): a refund and its later update share the refund id, so using it would make the update look like a conflicting redelivery and reject it. The same ID with identical data returns success without creating another event; changed immutable data returns `409` (including a redelivery of an event recorded before `payment_id` was required). Two simultaneous first deliveries of one ID can also produce a `409` for the loser; re-sending it returns the idempotent `200`. Events are linked to a client when possible, by exact match on a stored Whop user id (Accelerator membership or client card). The link is opportunistic and nothing displays it today: the count is the product, so every settled event counts toward Units Refunded whether or not it matched, an unmatched event is the NORMAL case and raises nothing, and no client-level events are ever shown.

| Field | Type | Required | Notes |
| --- | --- | --- | --- |
| `event_id` | string | Yes | Whop/provider event ID, unique. |
| `event_type` | `refund` or `dispute` | Yes | Whop's event name is accepted too: anything before the first dot is read as the object type, so `refund.created` and `refund.updated` both mean `refund`. The suffix carries nothing we store, since whether a refund is new or updated is already in its status. An event type outside `refund`/`dispute` is a `400` naming the whole value sent. |
| `status` | string | Yes | Whop's own words. Refund: `pending`, `requires_action`, `succeeded`, `failed`, `canceled`. Dispute: `needs_response`, `under_review`, `won`, `lost`, `closed`, `other`. A status outside its event type's list is a `400` naming it. |
| `occurred_at` | ISO 8601 timestamp | Yes | Provider event time, not our receipt time; scorecards window on it. On Whop's envelope this is the envelope `timestamp` (when Whop emitted the event), deliberately NOT `data.created_at`: for an update event that field is the original object's creation time, which is earlier than the event and would stop a dispute outcome from cancelling the loss it follows. |
...
| `payment_id` | string | Yes | Provider payment ID. The key an undo is matched to its refund by; an event without one is rejected with `400`. |
| `amount` | number or formatted money string | No | A decimal in the currency, so `10.43` means $10.43. Reconciliation only, stored as cents. |
| `currency` | three-letter code | No | Reconciliation only. |

The response is always the same shape: `created`, `duplicates`, `conflicts`, `ignored`, and `ids`, plus `id` and `linked` (or `id` and `duplicate`) when the body held exactly one event. `201` when something new was recorded, `200` otherwise, including a body of nothing but dispute alerts. Batched messages are written one at a time and are NOT in a transaction: a `500` part-way through leaves the earlier messages recorded, and re-sending the whole array is safe because each is keyed by its own `event_id`. A `409` is returned only when nothing in the request could be recorded; a batch that also landed good events keeps them and reports `conflicts`, because a conflict is permanent and 409-ing the whole array would have the sender retry it forever.
